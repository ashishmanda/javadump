...
public class Simulation {
	...
	public static List<SimulationEvent> runSimulation(
		...
		Restaurant ratsies = new Restaurant(numTables);
		

		// Start up machines
		Machine[] machines = new Machine[4];
		machines[0] = new Machine(MachineType.fryer, FoodType.wings, machineCapacity);
		machines[1] = new Machine(MachineType.oven, FoodType.pizza, machineCapacity);
		machines[2] = new Machine(MachineType.grillPress, FoodType.sub, machineCapacity);
		machines[3] = new Machine(MachineType.fountain, FoodType.soda, machineCapacity);
		
		ratsies.initMachines(machines);
		
		// Let cooks in
		Thread[] cooks = new Thread[numCooks];
		for(int i = 0; i < cooks.length; i++) {
			cooks[i] = new Thread(new Cook("Cook " + i, ratsies));
		}
		...
		if (!randomOrders) {
			...
			for(int i = 0; i < customers.length; i++) {
				customers[i] = new Thread(
						new Customer("Customer " + (i), order, ratsies)
						);
			}
		}
		else {
			for(int i = 0; i < customers.length; i++) {
				...
				customers[i] = new Thread(
						new Customer("Customer " + (i+1), order, ratsies)
				);
			}
		}
