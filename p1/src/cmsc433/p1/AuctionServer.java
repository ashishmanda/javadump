...
public class AuctionServer
{
	...
	//CHANGED: int --> INTEGER
	private Integer lastListingID = -1; 
	...
	/*
	  	This is the global lock order that needs to be preserved
	  	
	 	synchronized(itemsUpForBidding){
			synchronized(lastListingID){
				...
				synchronized(itemsAndIDs){		
					synchronized(itemsPerSeller){	
						synchronized(itemsSold){
							...
							int liveItems = itemsAndIDs.size() - soldItemsCount();
							
							if(liveItems == serverCapacity) return -1;
								
							Integer selling = itemsPerSeller.get(sellerName);
								
							if(selling == null) {
								itemsPerSeller.put(sellerName, new Integer(0));
								selling = 0;
							}
							
							if(selling == maxSellerItems) return -1;
							...
							itemsPerSeller.put(sellerName, selling++);
							//System.out.println( sellerName + " listed item #" + lastListingID );
							return new Integer(lastListingID);
						}
					}			
				}
			}
	...
	public List<Item> getItems()
	{
		...
		synchronized(itemsUpForBidding) {
			List<Item> liveItems= new ArrayList<Item>();
			...
			for(Item i : itemsUpForBidding ) {
				if(i.biddingOpen()) {
					...
					liveItems.add(i);
					//deep copy
					//liveItems.add(new Item(i.seller(), i.name(), i.listingID(), i.lowestBiddingPrice(), i.biddingDurationMs()));
				}
			}
	 		...
	 		synchronized(itemsAndIDs){
	 			synchronized(highestBids){
	 				synchronized(highestBidders){
	 					synchronized(blacklist){
	 						synchronized(itemsPerBuyer){
	 							//System.out.println( bidderName + " is attempting to place a bid for item #" + listingID );
	 							if(!itemsAndIDs.containsKey(listingID)) return false;
	 							
	 							Item desiredItem = itemsAndIDs.get(listingID);
	 							
	 							if( !desiredItem.biddingOpen() ||
//	 									itemsPerBuyer.get(bidderName) == maxBidCount ||
	 									blacklist.contains(bidderName)) return false;
	 							...
	 							if(lastBidder != null ) itemsPerBuyer.put(lastBidder, itemsPerBuyer.get(lastBidder) - 1);
					...
					synchronized(highestBidders){
						synchronized(itemsPerSeller){
							synchronized(itemsPerBuyer){
								synchronized(itemsSold){
									synchronized(instanceLock) {
										//System.out.println( bidderName + " is attempting to check on item #" + listingID );
										if(!itemsAndIDs.containsKey(listingID)) return FAILURE;
										if(itemsAndIDs.get(listingID).biddingOpen()) return OPEN;
										...
										if(itemsUpForBidding.contains(itemToCheck)) {
											...
											if(winningBidder != null) {
												//System.out.println(winningBidder + " is the winning bidder");
												uncollectedRevenue += highestBids.get(listingID);
												
												if(itemsPerBuyer.get(winningBidder) == -1) return FAILURE;  
												itemsPerBuyer.put(winningBidder, itemsPerBuyer.get(winningBidder) - 1);
												
											} else return FAILURE;
											//System.out.println("Bid ended, uncollected revenue now " + uncollectedRevenue + " dollars." );
										}
										
										
										if(!winningBidder.equals(bidderName)) return FAILURE;
										return SUCCESS;
									}
								}
							}
	...
	public int itemPrice(int listingID)
	{
		...
		synchronized(itemsAndIDs){
			synchronized(highestBids){
				...
				if(!itemsAndIDs.containsKey(listingID)) return -1;
				
				if(highestBids.containsKey(listingID)) return new Integer(highestBids.get(listingID));
				
				return new Integer (itemsAndIDs.get(listingID).lowestBiddingPrice()); 
	...
	public boolean itemUnbid(int listingID)
	{
		...
		synchronized(itemsAndIDs){
			synchronized(highestBids){
				if(!itemsAndIDs.containsKey(listingID)) return true;
				
				if(!highestBids.containsKey(listingID)) return true;
						...
						synchronized(itemsSold){
							synchronized(instanceLock){
								...
								if(!itemsAndIDs.containsKey(listingID) || 
										...
										blacklist.contains(bidderName)) return null;
								
								int purchaseAmt = highestBids.get(listingID);
								//System.out.println(bidderName+ " is trying to pay for #" + listingID);
								if(purchaseAmt > amount){
									//System.out.println(bidderName+ " couldn't pay for item #" + listingID+ ", cancelling all active bids now");
									for(int i= 0; i < lastListingID; i++) {
										//System.out.println("comparing " + highestBidders.get(i) + " to " + bidderName );
										if(!itemUnbid(i) && highestBidders.get(i).equals(bidderName)) {
								...
								soldItemsCount++;
								revenue+= amount;
								uncollectedRevenue-=purchaseAmt;
								
								//System.out.println("Revenue: " + revenue + " Uncollected Revenue: " + uncollectedRevenue);
