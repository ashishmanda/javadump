...
public class Command {
	...
	protected TreeSet<PMQuadtree> metropoli = new TreeSet<PMQuadtree>();
	/**
	...
	public void processCommands(final Element node) {
		...
		if (pmOrder == 3) {
			pmQuadtree = new PM3Quadtree(remoteSpatialWidth, remoteSpatialHeight);
		}
	...
	public void processCreateCity(final Element node) {
		...
		/* create the city */
		final City city = new City(name, remoteX, remoteY, radius, color, localX, localY);
		...
		if (!citiesByName.containsKey(start)) {
			...
			try {
				...
				if (Inclusive2DIntersectionVerifier.intersects(citiesByName
						.get(start).localToPoint2D(), new Rectangle2D.Float(0, 0,
						remoteSpatialWidth, remoteSpatialHeight))
						&& Inclusive2DIntersectionVerifier.intersects(
								citiesByName.get(end).localToPoint2D(),
								new Rectangle2D.Float(0, 0, remoteSpatialWidth,
										remoteSpatialHeight))) {
			...
			if (path == null) {
				...
				if (cityList.size() > 1) {
					...
					while (!cityList.isEmpty()) {
						...
						arc.setArcByTangent(city1.localToPoint2D(),
								city2.localToPoint2D(), city3.localToPoint2D(), 1);
	...
	private void saveShortestPathMap(final String mapName,
		...
		/* initialize map */
		map.setFrameSize(remoteSpatialWidth, remoteSpatialHeight);
		/* add a rectangle to show where the bounds of the map are located */
		map.addRectangle(0, 0, remoteSpatialWidth, remoteSpatialHeight, Color.BLACK, false);
	...
	private CanvasPlus drawPMQuadtree() {
		...
		/* initialize canvas */
		canvas.setFrameSize(remoteSpatialWidth, remoteSpatialHeight);

		/* add a rectangle to show where the bounds of the map are located */
		canvas.addRectangle(0, 0, remoteSpatialWidth, remoteSpatialHeight, Color.BLACK,
				false);
			...
			for (int i = 0; i < 4; i++) {
				...
				if (kid.getType() == Node.BLACK) {
					...
					if (c != null
						...
						double dist = point.distance(c.localToPoint2D());
			...
			for (int i = 0; i < 4; i++) {
				...
				if (kid.getType() == Node.BLACK) {
					...
					if (c != null && !road.contains(c)) {
						double dist = road.toLine2D().ptSegDist(c.localToPoint2D());
						nearCities.add(new NearestSearchRegion(kid, dist, c));
					}
				} else if (kid.getType() == Node.GRAY) {
