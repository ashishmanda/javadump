...
public abstract class PMQuadtree {
	...
	public class Black extends Node {
		...
		public Node add(final Geometry g, final Point2D.Float origin,
			...
			if (g.isRoad()) {
				...
				if (Inclusive2DIntersectionVerifier.intersects(r.getStart().localToPoint2D(), rect)) {
					addGeometryToList(r.getStart());
				}
	
				/* check if end point intersects with region */
				if (Inclusive2DIntersectionVerifier.intersects(r.getEnd().localToPoint2D(), rect)) {
			...
			for (int i = 0; i < 4; i++) {
				if (g.isRoad() && Inclusive2DIntersectionVerifier.intersects(
						...
						|| g.isCity() && Inclusive2DIntersectionVerifier.intersects(
								((City)g).localToPoint2D(),regions[i])) {
	...
	public void addRoad(final Road g) 
		...
		allRoads.add(g);
		if (Inclusive2DIntersectionVerifier.intersects(g.getStart().localToPoint2D(), world)) {
			increaseNumRoadsMap(g.getStart().getName());
		}
		if (Inclusive2DIntersectionVerifier.intersects(g.getEnd().localToPoint2D(), world)) {
	...
	public void addIsolatedCity(final City c) 
		...
		if (!Inclusive2DIntersectionVerifier.intersects(c.localToPoint2D(), 
